	...
	*/
	...
	class jzRawBackend extends jzBackendClass { 
		...
		*/
		...
		function install() {
			global $backend, $version, $include_path, $jzSERVICES,
			$sql_type, $sql_socket, $sql_db, $sql_usr, $sql_pw;
			...
			switch ($step) {
				case "init":
				global $word_database_user, $word_database_pass, $word_database_name, $word_database_server, $word_database_type,
					   $word_database_user_help, $word_database_pass_help, $word_database_name_help, $word_database_server_help, 
					   $word_database_type_help, $cms_type, $cms_mode, $word_cms_db_pick, $word_confirm_password, $word_create_database,
					   $word_false, $word_true, $word_create_database_help, $word_drop, $word_drop_database_help;
				  ?>
					...
					<table width="100%" cellspacing="0" cellpadding="3" border="0">
						...
						<tr>
							<td class="td" width="30%" align="left" valign="top">
								<?php echo $word_database_type; ?>
							</td>
							<td width="1">&nbsp;</td>
							<td width="70%" align="left">
								<select name="type" style="width:115px" onmouseover="return overlib('<?php echo $word_database_type_help; ?>', FGCOLOR, '#eeeeeee');" onmouseout="return nd();">
									<option value="DBX_MYSQL" selected>MySQL
									<option value="DBX_PGSQL">PostgreSQL
									<option value="DBX_SQLITE">SQLite
									<option value="DBX_MSSQL">Microsoft SQL Server
									<option value="DBX_ODBC">ODBC
									<option value="DBX_FBSQL">FrontBase
									<option value="DBX_OCI8">Oracle (OCI8)
									<option value="DBX_SYBASECT">Sybase-CT
								</select>
								<!--&nbsp; <a class="helpbox2" href="javascript:void(0);" onmouseover="return overlib('<?php echo $word_database_type_help; ?>');" onmouseout="return nd();">?</a>-->
							</td>
						</tr>
						<?php
				...
				break;
				 ...
				 case "db":
					...
					$fname = $this->data_dir . '/constants.php';
					@unlink($fname);
					
					if (!$handle = @fopen($fname,'w')) {
						touch($this->data_dir . '/constants.php');
					}
					if (!fopen($fname,'w')) {
						echo "could not write $this->data_dir /constants.php";
						return -1;
					}
					...
					if ($_POST['create_db'] == "drop"){
						...
						@unlink($include_path. "data/database/backend");
					}
					...
					if ($_POST['create_db'] == "drop" or $_POST['create_db'] == "true" or $_POST['create_db'] == "false"){
						...
						if ($sql_type != "DBX_MYSQL") {
							$data = file_get_contents($include_path. "install/jinzora.ms.sql");
						} else {						
